...
def get_config():
    ...
    tl.files.exists_or_mkdir(config.model.model_dir+"/partial_results", verbose=True)  # to save partial results
    tl.files.exists_or_mkdir(config.model.model_dir+"/partial_results/images", verbose=True)
    ...
    tl.files.exists_or_mkdir(os.path.dirname(config.log.tensorboard_train_path), verbose=True)
...
|       file path to save train log                  ./save_dir/model_name/log.txt
    ...
    update_log.tensorboard_train_path = f"./save_dir/{update_model.model_name}/tensorboard/" + current_time + "/train"
...
def set_privacy_preserving_model(privacy_preserving_model):
    update_model.privacy_preserving_model = privacy_preserving_model


def set_get_data_from_tfrecord(get_from_tfrecord):
    update_data.get_data_from_tfrecord = get_from_tfrecord


def set_config_optics_file(optics_file_path):
    update_model.optics_file_path = optics_file_path
