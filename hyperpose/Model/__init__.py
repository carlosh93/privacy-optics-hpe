...
def get_model(config):
    ...
    # using defualt arch
        ...
        if (
                model_type == MODEL.Openpose or model_type == MODEL.LightweightOpenpose or model_type == MODEL.MobilenetThinOpenpose):
        ...
        if model_type == MODEL.Openpose:
            ...
            ret_model = model_arch(parts=model.parts, n_pos=len(model.parts), limbs=model.limbs,
                                   n_limbs=len(model.limbs), num_channels=model.num_channels, \
                                   hin=model.hin, win=model.win, hout=model.hout, wout=model.wout, backbone=backbone,
            ...
            ret_model = model_arch(parts=model.parts, n_pos=len(model.parts), limbs=model.limbs,
                                   n_limbs=len(model.limbs), num_channels=model.num_channels, hin=model.hin,
                                   win=model.win, \
                                   hout=model.hout, wout=model.wout, backbone=backbone, pretraining=pretraining,
            ...
            ret_model = model_arch(parts=model.parts, K_size=len(model.parts), limbs=model.limbs,
                                   L_size=len(model.limbs), hnei=model.hnei, wnei=model.wnei, lmd_rsp=model.lmd_rsp, \
                                   lmd_iou=model.lmd_iou, lmd_coor=model.lmd_coor, lmd_size=model.lmd_size,
                                   lmd_limb=model.lmd_limb, backbone=backbone, \
                                   pretraining=pretraining, data_format=model.data_format)
    ...
    # Make model private
    if config.model.privacy_preserving_model:
        from .openpose.model import Private_OpenPose as private_model_arch
        ret_model = private_model_arch(optics_config_file_path=config.model.optics_file_path,
                                       pose_model=ret_model,
                                       config = config)
    return ret_model
